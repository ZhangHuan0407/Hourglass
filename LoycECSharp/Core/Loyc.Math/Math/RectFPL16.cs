...
namespace Loyc.Math
{
	public struct RectFPL16
	{
		...
		public Vector2FPL16 Center
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get => new Vector2FPL16((XMin + XMax) / 2L, (YMin + YMax) / 2L);
		}

		public Vector2FPL16 LeftTop
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get => new Vector2FPL16(XMin, YMax);
		}
		
		public Vector2FPL16 RightTop
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get => new Vector2FPL16(XMax, YMax);
		}
		
		public Vector2FPL16 LeftBottom
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get => new Vector2FPL16(XMin, YMin);
		}
		
		public Vector2FPL16 RightBottom
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get => new Vector2FPL16(XMax, YMin);
		}
		...
		public void Offset(Vector2FPL16 offset)
		{
			XMin += offset.x;
			YMin += offset.y;
			XMax += offset.x;
		...
		public void SizeMultiplyScale(Vector2FPL16 scale)
		{
			FPL16 xCenter = (XMin + XMax) / 2L;
			FPL16 width = (XMax - XMin) * scale.x;
			XMin = xCenter - width;
			XMax = xCenter + width;

			FPL16 yCenter = (YMin + YMax) / 2L;
			FPL16 height = (YMax - YMin) * scale.y;
			YMin = yCenter - height;
			YMax = yCenter + height;
		}
