...
namespace Loyc.Math
{
    public static class FPL16TriangleFunctionExtension
    {
		...
		/// <summary>
		/// calculate radian - anotherRadian, and constrain the results to interval [-Π, Π]
		/// </summary>
		/// <param name="radian">[-Π, Π]</param>
		/// <param name="anotherRadian">[-Π, Π]</param>
		/// <returns>[-Π, Π]</returns>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static FPL16 MinusRadian(FPL16 radian, FPL16 anotherRadian)
		{
			FPL16 result = radian - anotherRadian;
			if (result > FPL16.Zero)
				return (result + PI) % PI2 - PI;
			if (result < FPL16.Zero)
				return (result - PI) % PI2 + PI;
			return FPL16.Zero;
		}
		...
		/// <returns>[-Π, Π]</returns>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static FPL16 PlusRadian(FPL16 radian, FPL16 anotherRadian)
		{
			FPL16 result = radian + anotherRadian;
			if (result > FPL16.Zero)
				return (result + PI) % PI2 - PI;
